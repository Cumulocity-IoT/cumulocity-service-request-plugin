# Cumulocity Service Request Plugin Specification

## Requirements

Functional requirements below were derived from the current implementation under `src/service-request-plugin`. Each requirement has a unique, stable identifier (`FR-XXX`) that should be kept even if requirements are reordered; new requirements should append the next free number.

This document has two parts:

1. **Baseline (current behavior)** — FR-001 to FR-062, describing what is implemented today.
2. **Target redesign** — FR-063 onward, describing the new combined alarm/service-request timeline and split-view experience. Requirements here supersede specific baseline requirements where noted; baseline requirements not mentioned as superseded remain in force.

### Plugin registration & availability

- **FR-001**: The plugin shall register a "Service Requests" navigation tab in the device context (`ViewContext.Device`), routed at path `service-requests`, showing the `online-support` icon.
- **FR-002**: The Service Requests tab shall only activate if the `service-request-mgmt` microservice is reachable (`HEAD /service/service-request-mgmt/api/service/request/`); the availability check result shall be cached for the lifetime of the guard instance.
- **FR-003**: If the microservice is not available, the plugin shall show an informational alert instructing the user to install the `service-request-mgmt` microservice.

### Service request dashboard

- **FR-004**: Navigating to the Service Requests tab for a device shall render a dashboard pre-configured with two widgets: an Alarm List Widget and a Service Request List Widget, both scoped to the current device.
- **FR-005**: The dashboard shall expose an action to open the service request creation modal for the current device (not tied to a specific alarm).

### Alarm list widget

- **FR-006**: The widget shall list alarms for the configured device, including alarms of sub-assets/sub-devices when the widget is configured to show subassets.
- **FR-007**: The widget shall support filtering the alarm list by one or more alarm statuses (e.g. ACTIVE, ACKNOWLEDGED, CLEARED) and by one or more severities (CRITICAL, MAJOR, MINOR, WARNING), reloading results whenever a filter changes.
- **FR-008**: The widget shall order results by severity ascending, then time descending, then text ascending by default, and additionally by status when configured with `order: 'status'`.
- **FR-009**: The widget shall display a severity icon and a status icon per alarm, and an occurrence-count badge when an alarm's count exceeds 1.
- **FR-010**: The widget shall support paginated loading of additional alarms ("load more") without discarding already-loaded alarms.
- **FR-011**: The widget shall support automatic polling for new alarms on a fixed interval (60s) that can be toggled on/off by the user; a manual reload action shall also be available.
- **FR-012**: Loading additional pages or reloading the list shall temporarily suspend polling and restore it once the load completes, if polling is enabled.
- **FR-013**: The widget shall provide an action to clear an ACTIVE alarm (setting its status to CLEARED), showing a success or error notification and reloading the alarm list on success.
- **FR-014**: The widget shall provide an action, visible only for alarms with status ACTIVE, to open the service request creation modal pre-associated with that alarm.
- **FR-015**: Each alarm entry shall link to its source device's page.
- **FR-016**: The widget shall show a loading indicator while alarms are being fetched, and an empty-state message when no alarms match the current filters.

### Service request list widget

- **FR-017**: The widget shall list service requests for the configured device (by source device ID), with a default page size of 500.
- **FR-018**: The widget shall automatically reload its list on a fixed interval (180s) and whenever a service request is created, updated, or resolved elsewhere in the plugin.
- **FR-019**: Each list entry shall display the request's priority icon and current status; entries whose status equals the closed status (`10`) shall be visually distinguished as closed.
- **FR-020**: Each list entry shall be expandable/collapsible to reveal its full description (or a "no description provided" message when absent) and its comments.
- **FR-021**: Expanding an entry for the first time shall lazily load up to a configurable number of comments (default 10) for that request, plus a count of any additional comments not shown.
- **FR-022**: Each list entry shall link to its source device's page.
- **FR-023**: Clicking a service request's title shall open the service request edit modal for that request.
- **FR-024**: The widget shall show a loading indicator while service requests are being fetched, and an empty-state message when none exist.
- **FR-025**: The widget shall be configurable and registered as a dashboard widget (`service-request.list.widget`) selectable without requiring a specific device target.

### Service request creation & editing modal

- **FR-026**: The modal shall support two modes: creating a new service request (for a device, optionally associated with an alarm) and editing an existing service request.
- **FR-027**: On open, the modal shall fetch and cache available statuses and priorities via the meta service, used to populate the status and priority form controls.
- **FR-028**: When creating a request from an alarm, the modal shall pre-fill the title with the alarm's text and associate the request with the alarm via `alarmRef` (URI + ID); when creating for a device without an alarm, no alarm reference shall be set.
- **FR-029**: When creating a request, the source device (ID, self link, name) shall be attached to the request automatically.
- **FR-030**: When creating a request, the default status and default (first available) priority shall be pre-selected.
- **FR-031**: The form shall require a non-empty title; description, priority, and attachment shall be optional.
- **FR-032**: The status field shall be editable when creating a new request but disabled (read-only) when editing an existing request.
- **FR-033**: Submitting the create form shall send a create request (`title`, `status`, `priority`, `description`, `type`, `alarmRef`, `eventRef`, `seriesRef`, `source`, `customProperties`) and, on success, show a success notification and refresh the modal with the newly created request's details.
- **FR-034**: Submitting the edit form shall send an update request containing only the changed fields (`title`, `priority`, `description`) and, on success, show a success notification and refresh the modal with the updated request's details.
- **FR-035**: The edit form's submit action shall be disabled while the form is pristine (unmodified), invalid, or a submission is already in progress.
- **FR-036**: The modal shall provide a "Reset" action in edit mode that restores the form to the last-loaded values and clears the dirty/pristine state.
- **FR-037**: The detail panel shall provide a "Close" action, available only while the request is active and not already closed, that opens a picker listing the tenant's configured closing statuses (`status.isClosedTransition: true`, via `GET /request/status`) and applies the status the user picks (`PUT /request/{id}` with `status`), letting the microservice cascade its configured `alarmStatusTransition` to the linked alarm(s). The action is disabled, with an explanatory tooltip, when the tenant has no status configured with `isClosedTransition: true`. (ADR-0002; supersedes the prior "Resolve" action, which only sent `isActive: false` and never closed the request or its linked alarm.)
- **FR-038**: When the loaded service request is closed (`isClosed: true`), the entire form shall be disabled and the Close action shall be unavailable.
- **FR-039**: On successful create, update, or close, the modal shall close and notify listeners (e.g. list widgets) that a change occurred so they can refresh.
- **FR-040**: The modal shall provide a "Cancel" action that closes the modal without submitting changes.
- **FR-041**: The modal shall present tabbed navigation for Measurements, Events, and Alarms related to the request; the Alarms tab shall be active by default while Measurements and Events remain placeholders (disabled) in the current implementation.
- **FR-042**: The modal shall allow attaching a single file to the service request via drag-and-drop or file picker; selecting a new file while one is already attached shall mark the previous attachment for deletion and stage the new one, since the backend currently supports only one attachment per request.
- **FR-043**: On successful creation or update, any staged new attachment shall be uploaded to the newly created/updated request.
- **FR-044**: The modal shall allow downloading the current attachment of a service request.
- **FR-045**: The modal shall embed the comments component (see below) so users can view and add comments while editing a request.

### Service request comments

- **FR-046**: Comments for a service request shall be fetched and displayed in reverse order as returned by the API, limited to a configurable number of displayed items with a count of any remaining, hidden comments.
- **FR-047**: Users shall be able to submit a new text comment (`type: 'user'`) for a service request; the text field is required and the submit action shall be a no-op while a submission is already in progress.
- **FR-048**: After a comment is successfully created, the comment form shall reset and the comment list shall be refreshed.
- **FR-049**: Users shall be able to attach a single file to a new comment; the attachment shall be uploaded in a follow-up request using the newly created comment's ID, after the comment itself is created.
- **FR-050**: Users shall be able to download a comment's attachment, if present.
- **FR-051**: Comment text containing newline characters shall render with line breaks (`<br>`) in the UI.
- **FR-052**: System-generated comments (`type: 'system'`) shall be visually distinguishable from user comments.

### Service request & comment attachments (device-agnostic)

- **FR-053**: Uploading an attachment shall send the file as multipart form data along with a `force` flag indicating whether an existing attachment should be overwritten.
- **FR-054**: Downloading an attachment shall retrieve the raw file content and offer it to the user as a browser file download (save-as).
- **FR-055**: Attachment upload and download failures shall surface a localized error notification, including any server-provided error message when available.

### Device & alarm selection components

- **FR-056**: The device selection component shall list devices (`c8y_IsDevice`) with live updates to the currently selected device's properties (via realtime subscription), and support text filtering of the candidate list.
- **FR-057**: The alarm selection component shall support selecting a single alarm or multiple alarms by reference, resolving each reference to full alarm details (using a locally cached list of available alarms plus on-demand detail fetches for any not already cached), and allow removing a selected alarm from the selection.
- **FR-058**: If a referenced alarm cannot be found/fetched, the alarm selection component shall show an error notification rather than failing silently.

### Data & API integration

- **FR-059**: The plugin shall communicate with the `service-request-mgmt` microservice via the following REST operations: list/detail/create/update/resolve service requests; list statuses; list priorities; list/create comments; upload/download request attachments; upload/download comment attachments. REST API details are documented in [https://github.com/Cumulocity-IoT/cumulocity-microservice-service-request-mgmt/blob/develop/docs/openapi.json](https://github.com/Cumulocity-IoT/cumulocity-microservice-service-request-mgmt/blob/develop/docs/openapi.json)
- **FR-060**: A service request shall carry: id, title, optional description, status, optional priority, active flag, timestamps (creation/update/last-updated), owner, type (currently always `'alarm'`), optional source device reference, optional alarm/event/series references, optional custom properties, and an optional attachment.
- **FR-061**: List and comment-list requests shall default to a page size of 500 when no explicit limit is provided.
- **FR-062**: All create/update/resolve/comment operations shall surface success or failure via user-facing notifications (success on completion, danger/error with server status text on failure), and log details to the console for diagnostics.

## Target redesign

The redesign aligns the plugin with Cumulocity's standard alarm dashboard conventions (`AlarmListWidgetComponent` / `AlarmsListComponent`, the `c8y-list-group` primitives, and the standard split-view layout pattern), while introducing a combined alarm + service-request timeline. Reference material used for alignment: `@c8y/ngx-components` alarm widget/list types (SDK 1024.8.3), the Codex design-system docs for split view, list-group, badges, and feedback severity, and a live walkthrough of the standard alarm details page.

A high-fidelity HTML/CSS mockup implementing everything below lives at [`docs/design/service-request-timeline-mockup.html`](./design/service-request-timeline-mockup.html) (open it directly in a browser). Requirements in this section were iterated against that mockup and reflect its final state, not the first draft; unlike the baseline section, FR text here has been edited in place rather than superseded, since none of it is implemented yet.

### Combined timeline (master list)

- **FR-063**: The alarm list widget and service request list widget (FR-004, FR-006–FR-025) shall be replaced by a single combined timeline listing both alarms and service requests for the device, ordered newest-first, using the standard `c8y-list-group` timeline list group variant (`c8y-li-timeline` wrapping each row's date/time and its `c8y-li` content) instead of custom bootstrap markup.
- **FR-064**: The alarm-to-service-request relationship is one service request to many alarms: a service request references zero or more alarms via its `alarmRefList` (each entry's `id` matching an alarm's `id`), while a given alarm shall be linked to at most one service request (the reverse — multiple service requests referencing the same alarm — is not supported and shall not occur). For each alarm a service request references, its box shall be rendered side by side with that alarm's row (not stacked below it), connected by a small arrow that also displays the elapsed time between the alarm and the request; where a request references more than one alarm, one arrow is drawn per referenced alarm, from the single request box to each. The arrow is decorative only (not independently clickable — clicking either box selects it). This supersedes the singular `alarmRef` framing in FR-028/FR-033/FR-060 — the redesign uses the current (non-deprecated) `alarmRefList` array field instead of the deprecated singular `alarmRef`.
- **FR-065**: If a service request has an empty `alarmRefList` (created directly from a device), it shall appear as its own standalone timeline entry with no connecting arrow and no paired alarm box.
- **FR-066**: Since an alarm can be linked to at most one service request (FR-064), once an alarm has a linked service request its dashed "Create service request" placeholder (FR-067) shall no longer be offered for that alarm — the placeholder is only ever shown for alarms with no linked request.
- **FR-067**: An alarm with no linked service request shall show an empty, dashed placeholder box directly beside it in the timeline (in place of a service-request box) offering two call-to-action buttons — "Create service request" and "Add existing service request" (FR-085) — so a request can be created or linked without leaving the timeline.
- **FR-068**: By default, the timeline shall show only relevant entries, per the following relevance rule (see [ADR-0001](./adr/0001-timeline-relevance-filter.md)): an entry is hidden when it is (a) an alarm that is `CLEARED` with no linked service request, (b) an alarm-plus-service-request pair where the alarm is `CLEARED` and the service request is closed (`isClosed`), or (c) a standalone service request (no `alarmRefList` entries, FR-065) that is closed. Everything else is shown by default — any `ACTIVE`/`ACKNOWLEDGED` alarm regardless of a linked request, any pair whose service request is still open regardless of the alarm's status, and any open standalone service request. The action bar shall provide a single "Show resolved" toggle, off by default, that when enabled disables the relevance rule and shows every entry unfiltered, including cleared orphan alarms and closed pairs. This replaces the severity/date/alarm-type filters (FR-007) for the timeline, which no longer apply here — those filters were alarm-only and had no way to express relevance for service requests, requiring the ad-hoc "always show non-closed requests" exception this FR now supersedes.
- **FR-069**: The timeline shall continue to show a severity icon and status icon per alarm using the standard severity-to-icon mapping (CRITICAL → `exclamation-circle`, MAJOR → `warning`, MINOR → `high-priority`, WARNING → `circle`) and an occurrence-count badge for alarms with `count > 1`, per the standard badge pattern.
- **FR-070**: Each service-request box shall show a distinct duo-color "c8y-support" icon (visually distinguishing it from the alarm's severity-badged icon), the request's status and priority pills, and its type (see FR-077); closed requests shall remain visually distinguished (FR-019).

### Split view & detail panel

- **FR-071**: The Service Requests tab shall use the standard split-view layout with a 50/50 default column split: the combined timeline occupies the list column, and a detail panel occupies the second column, replacing the modal-based creation/editing flow (supersedes the "modal" framing in FR-026, FR-039, FR-040, and the dashboard action in FR-005).
- **FR-072**: Selecting an alarm row in the timeline shall display that alarm's details in the detail panel, mirroring the standard alarm details view (status, severity, source, type, occurrence count, first occurrence, custom data, and an audit-log/history list).
- **FR-073**: Selecting a service-request box shall display that service request's details in the detail panel, organized into five sections in this order: **Service Request** (an alarm-reference banner per linked alarm, opened-by/source, title, description, type, status, priority), **Field Service Status** (FR-079), **Attachment** (FR-084), **Comments** (FR-080), and **Raw JSON** (FR-081). This supersedes the tabbed Measurements/Events/Alarms navigation in FR-041 — those tabs are removed as no longer needed.
- **FR-074**: Only one timeline entry (an alarm or a service-request box) shall be active/selected at a time; selecting a new entry shall switch the detail panel's contents accordingly, per the standard split-view single-selection behavior.
- **FR-075**: The detail panel shall follow standard split-view conventions: sticky header and footer (action buttons), and a "Back" affordance to return to the list on small/tablet screens.
- **FR-076**: Creating a new service request shall populate the detail panel with the creation form rather than opening a modal. When opened from a specific alarm's "Create service request" action, the alarm reference is implied (shown via a banner) and the form does not show reference pickers. When opened from the device-level action in the action bar, the form additionally offers a Type selector (FR-077) and two optional reference pickers — Alarm reference (supporting selection of one or more alarms, per FR-057 and the one-service-request-to-many-alarms relationship in FR-064) and Event reference — so a manually-created request can optionally be linked to either.

### Service request type, status, and FSM-synced data

- **FR-077**: A service request's `type` shall be one of five enumerated values — Alarm, Note, Maintenance, Downtime, Other — set at creation and not editable afterward. It shall be shown as a distinct tag on the service-request box in the timeline, in the detail panel's eyebrow, and as a read-only field next to Status in the Service Request detail section. This supersedes the "(currently always `'alarm'`)" note in FR-060.
- **FR-078**: In the redesigned UI, the Status field shall always be read-only, in both creation and editing — status is set exclusively by the system/FSM integration and is never manually edited. This supersedes FR-032, which allowed editing status at creation. The one deliberate exception is the Close action's status picker (FR-037, ADR-0002): the user may choose which of the tenant's configured *closing* statuses (`isClosedTransition: true`) to apply when closing a request — this is not a general status edit, and does not reopen the Status field itself to editing.
- **FR-079**: The Field Service Status section shall show data synced from the FSM integration, corresponding to the service-request-mgmt data model's `field*`-prefixed attributes plus `fsmLink` and `externalId`: Assignee, Scheduled window (start–end), Due, Progress (percentage, shown with a progress bar), FSM task (link to the external FSM system), and External ID. All fields in this section are read-only.

### Comments, attachments, and raw data

- **FR-080**: Comments shall render as a chronological timeline (a time column, connecting spine, and entry text per comment) consistent with the alarm audit-log presentation, rather than as chat bubbles. The comment list is expanded by default and scrolls independently of the page when long, without affecting the fixed comment-compose row beneath it.
- **FR-081**: A raw JSON representation of the underlying object, collapsed by default with a copy-to-clipboard action, shall be available in the detail panel for service-request selections. Alarm selections do not show a raw JSON view.
- **FR-082**: The alarm detail panel's footer shall show "Create service request" and "Add existing service request" (FR-085) actions alongside Acknowledge/Clear when the alarm has no linked service request (replacing the separate inline banner and its own button, per FR-067). When the alarm already has a linked service request, neither action is shown — only the banner's "Open" action is shown; no duplicate "Open linked request" footer action.
- **FR-083**: The action bar shall display two read-only chips sourced from the device's managed object, separate from any individual alarm or service request: the Service Object ID (`sr_ServiceObjectId` fragment) and an active-service-request counter by priority (`sr_ActiveStatus` fragment, e.g. `{ "medium": 3 }`).
- **FR-084**: Attachments — one per service request (FR-042) and one per comment (FR-049) — shall use the `c8y-file-picker`/`c8y-drop-area` visual pattern (drop zone with "Upload file" / "Drop file here" text, or an attachment chip with name/size/download/remove once a file is present). This is a restyle only; upload/download behavior is unchanged.
- **FR-085**: "Add existing service request" (offered from the alarm's timeline placeholder per FR-067 and its detail-panel footer per FR-082) shall open a lookup of existing service requests for the device, filterable by title or ID via client-side text search — the service-request-mgmt list API has no server-side title-search query parameter, so filtering happens locally over the fetched candidate list (consistent with the client-side filtering pattern in FR-056). Selecting a candidate links the alarm to it by calling the microservice's dedicated single-reference endpoint (`PUT /service/request/{id}/alarm` with a `ServiceRequestDataRef` body) rather than resending the whole `alarmRefList`, establishing the one-service-request-to-many-alarms relationship (FR-064) for a request that already exists. On success, the alarm's placeholder is replaced by a paired service-request box (per FR-064), the alarm's detail view gains a "Linked to" banner and loses its FR-067/FR-082 actions (per FR-066), and — since the same request may now be paired with more than one alarm — every occurrence of that request's box in the timeline reflects the updated link count, and its detail view gains an additional alarm-reference banner (FR-073) for the newly linked alarm.

### Group/asset-level availability and global navigation

- **FR-086**: The Service Requests tab (FR-001) shall also register under `ViewContext.Group` — the same route Cumulocity uses for both groups and assets — reusing the same path, icon, component, and availability guard (FR-002) as the device-level tab. The combined alarm/service-request timeline (FR-063 onward) is therefore available, unmodified, on device, group, and asset detail pages alike.
- **FR-087**: Alarm and service-request queries shall use the current context's managed object as `source` together with `withSourceAssets`/`withSourceDevices` enabled, so that at group/asset context the timeline includes alarms and service requests from every descendant device, asset, and addition — no manual child-traversal is implemented client-side. This is not new behavior: `loadAlarms()` already sets both flags today (FR-006); they are a no-op for a leaf device and become the aggregation mechanism once the context is a group/asset. `ServiceRequestListRequest` (the client-side request model) shall be extended with `withSourceAssets`/`withSourceDevices` fields, and `loadServiceRequests()` shall set them, to reach the identically-named query parameters the service-request-mgmt microservice's `GET /api/service/request` endpoint already supports.
- **FR-088**: A new top-level "Service Requests" navigator node shall be registered in the left navigation bar (`hookNavigator`), alongside a new context-free route (`hookRoute`, no `context`) rendering the same combined timeline component and gated by the same microservice-availability guard (FR-002) — mirroring how the built-in Alarms application is available both as a per-context tab and as a standalone top-level list.
- **FR-089**: The top-level route shall default to an unscoped view — alarms and service requests across the whole tenant, subject to the user's normal inventory/alarm read permissions — matching the default behavior of the standard Alarms application, rather than requiring an upfront source selection.
- **FR-090**: The top-level view shall offer a source selector, implemented with the standard `c8y-asset-selector` component, letting the user scope the timeline down to a specific device, group, or asset (its descendants included, per FR-087) or clear the selection to return to the unscoped tenant-wide view.
- **FR-091**: Each timeline row shall display its source device's name whenever the current view is not scoped to a single device — group/asset context, the unscoped top-level view, or a broader source picked via FR-090 — extending the alarm box's existing source-name sub-line (already shown today, regardless of context) to the service-request box, which does not currently show it.
- **FR-092**: The "New service request" creation form shall show a required device selector (`c8y-asset-selector`, restricted to devices) before the Title/Description fields whenever it is opened without a single implicit device — i.e. from the group/asset tab's or the top-level view's "New service request" action-bar button. Opening the form from a specific alarm row is unaffected: the alarm's own `source` continues to supply the device implicitly (FR-028), regardless of which context the alarm was viewed from.
- **FR-093**: Once a target device is selected (FR-092) or is already implicit (device context, or created from a specific alarm), the Alarm reference and Event reference dropdowns shall list only that device's own alarms/events — matching today's single-device scoping — not the broader aggregate visible in the timeline.
- **FR-094**: The timeline's existing behaviors — the relevance-based "Show resolved" toggle (ADR-0001), pagination ("load more"), auto-refresh polling, and the cleared-alarm-linked-to-open-request reconciliation (`includeClearedAlarmsLinkedToOpenRequests`) — continue to apply unchanged at group/asset and top-level scope, since they already operate on the returned alarm/service-request arrays regardless of how broad the underlying query is.

### Open questions for the design mockup phase

- ~~What "alarm details" contains~~ — resolved by FR-072/FR-082.
- ~~Exact chronological ordering rule for the combined timeline~~ — resolved by FR-063 (newest-first).
- ~~Whether the connecting arrow is interactive~~ — resolved by FR-064 (decorative only).
- ~~Responsive behavior of the timeline/detail relationship on narrow (tablet/mobile) layouts~~ — resolved: this version targets desktop widths only; the mockup is built and verified at desktop widths, and tablet/mobile responsive behavior is deferred to a future version.
- ~~Scope of non-alarm request types~~ — resolved: this version focuses on the alarm-to-service-request flow end to end (FR-063–FR-076, FR-082). The Note, Maintenance, Downtime, and Other request types (FR-077) are enumerated and selectable, and Maintenance has one example in the timeline, but detailed handling of these non-alarm types (e.g. type-specific fields, distinct workflows) is intentionally deferred to a future version.
- ~~Whether `type` (FR-077) is truly immutable after creation~~ — resolved: confirmed with product/backend that `type` is immutable after creation; FR-077 stands as written.
- ~~The Event reference picker (FR-076) uses two placeholder demo events~~ — resolved: both the Alarm reference and Event reference pickers now use the standard `c8y-select` component, backed by the device's real alarms/events (fetched via `AlarmService`/`EventService`), showing each candidate's `text`.
- ~~How to render a service request that references multiple alarms which are not adjacent in the chronological timeline~~ — resolved by FR-085: the mockup's "Add existing service request" flow demonstrates linking SR-042 (already paired with alarm-1) to a second, non-adjacent alarm. The request's box repeats beside each alarm it references (rather than a single box with a long arrow), each occurrence's sub-line reads "Linked to N alarms," and the request's own detail view gains one alarm-reference banner per linked alarm (FR-073).
- Exact left-navigator priority/position for the new top-level "Service Requests" node (FR-089) relative to Alarms and other built-in nodes — not yet specified; will pick a reasonable value near Alarms during implementation unless directed otherwise.
- Whether the top-level unscoped view (FR-090) needs a raised/paginated fetch ceiling beyond the existing 2,000-per-request cap (ADR-0001's `RELEVANT_PAGE_SIZE`) on very large tenants — no data yet on the tenant sizes this needs to handle well; ships with the existing cap and "load more" pagination (FR-010/FR-018) unless a real large-tenant case surfaces problems.
- Whether a source picked via the top-level selector (FR-091) should be reflected in the URL (query param) for shareable/bookmarkable links, or stay transient in-memory state for this iteration — defaulting to transient state unless directed otherwise.
